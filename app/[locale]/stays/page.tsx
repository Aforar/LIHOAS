...
export default function StaysPage() {
    ...
    return (
        <main className="flex min-h-screen flex-col font-sans pt-32 pb-24">
            {/* Header */}
            <section className="text-center px-6 mb-16 md:mb-20 max-w-3xl mx-auto">
                ...
                <ScrollElement direction="up" delay={0.3}>
                    <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif text-white mb-4 md:mb-6">{t('title')}</h1>
                </ScrollElement>
                <ScrollElement direction="up" delay={0.4}>
                    <p className="text-lg md:text-xl text-text-muted font-light px-4 md:px-0">
                        {t('desc')}
                    </p>
                </ScrollElement>
            </section>
            ...
            {/* Rooms Overview */}
            <section className="px-6 lg:px-12 max-w-7xl mx-auto w-full mb-24 md:mb-32 flex flex-col gap-12 md:gap-16">
                {rooms.map((room, i) => (
                    <ScrollElement direction="up" delay={0.2} key={i}>
                        <div className={`group rounded-3xl overflow-hidden bg-white/5 border border-white/10 grid grid-cols-1 md:grid-cols-2 relative h-full md:h-[450px] ${i % 2 === 1 ? 'md:[direction:rtl]' : ''}`}>
                            ...
                            {/* Content Side */}
                            <div className="[direction:ltr] p-8 md:p-14 flex flex-col justify-center bg-gradient-to-br from-green-950/80 to-green-950/40 backdrop-blur-md relative z-10">
                                <div className="mb-4">
                                    <h3 className="text-2xl md:text-3xl font-serif text-white mb-2">{room.title}</h3>
                                ...
                                <p className="text-text-muted leading-relaxed mb-6 md:mb-8 flex-grow text-sm md:text-base">
                                    {room.desc}
                                </p>
                                <div>
                                    <a href="#booking" className="inline-block w-full md:w-auto">
                                        <GlassButton variant="primary" className="px-6 py-3 w-full md:w-auto text-center justify-center">{t('book_now')}</GlassButton>
                                    </a>
                                </div>
                            </div>
