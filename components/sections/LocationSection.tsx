        ...
        <section id="location" className="relative h-screen flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 w-full text-center">
                ...
                <ScrollElement direction="up" delay={0.4}>
                    <h3
                        className="text-5xl md:text-7xl font-serif text-white leading-tight mb-8 drop-shadow-lg"
                ...
                <ScrollElement direction="up" delay={0.7}>
                    <div className="flex flex-wrap justify-center gap-12 text-white font-sans text-sm tracking-widest uppercase">
                        <div className="flex flex-col items-center gap-2 bg-black/30 backdrop-blur-sm px-8 py-6 rounded-2xl border border-white/10">
                            <span className="text-4xl font-serif text-emerald-light normal-case">{t('m1_val')}</span>
                            <span className="text-text-muted text-xs">{t('m1_lbl')}</span>
                        </div>
                        <div className="flex flex-col items-center gap-2 bg-black/30 backdrop-blur-sm px-8 py-6 rounded-2xl border border-white/10">
                            <span className="text-4xl font-serif text-emerald-light normal-case">{t('m2_val')}</span>
                            <span className="text-text-muted text-xs">{t('m2_lbl')}</span>
                        </div>
                        <div className="flex flex-col items-center gap-2 bg-black/30 backdrop-blur-sm px-8 py-6 rounded-2xl border border-white/10">
                            <span className="text-4xl font-serif text-emerald-light normal-case">{t('m3_val')}</span>
                            <span className="text-text-muted text-xs">{t('m3_lbl')}</span>
                        </div>
                    </div>
                </ScrollElement>
            </div>
